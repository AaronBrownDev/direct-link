...
CameraCapture::~CameraCapture() {
    ...
    if (pipeline_ != nullptr) {
        ...
        // failed mid-way), the pipeline may still own live GStreamer threads that
        // would keep the process alive after the window is closed.
...
Result CameraCapture::buildPipeline() {
    ...
    const std::string rawCaps =
        "video/x-raw,format=I420"
        ",width="     + std::to_string(w) +
        ",height="    + std::to_string(h) +
        ",framerate=" + std::to_string(f) + "/1";
    const std::string sinkProps =
        "appsink name=sink sync=false max-buffers=4 drop=true emit-signals=false";

    // Ordered candidates. Each is probed to READY state; the first that succeeds
    // is kept. READY is sufficient to check device availability — it avoids the
    // cleanup hang caused by stopping a live streaming thread mid-flight.
    //
    ...
    //    pipewiresrc requires the session manager (WirePlumber) to have registered
    //    video nodes; when no nodes exist it hangs indefinitely at PLAYING.
    //  - pipewiresrc variants are kept as fallbacks for environments where native
    ...
    candidates.push_back(
        ...
        ",height="    + std::to_string(h) +
        ",framerate=" + std::to_string(f) + "/1"
        " ! jpegdec ! videoconvert ! video/x-raw,format=I420 ! " + sinkProps);
    ...
    candidates.push_back(
        ...
        ",height="    + std::to_string(h) +
        ",framerate=" + std::to_string(f) + "/1"
        " ! videoconvert ! video/x-raw,format=I420 ! " + sinkProps);

    GstElementFactory *pwFactory = gst_element_factory_find("pipewiresrc");
    if (pwFactory != nullptr) {
        gst_object_unref(pwFactory);
        candidates.push_back(
            "pipewiresrc target-object=" + dev + " do-timestamp=true"
            " ! jpegdec ! videoconvert ! videoscale ! " + rawCaps + " ! " + sinkProps);
        candidates.push_back(
            "pipewiresrc do-timestamp=true"
            " ! jpegdec ! videoconvert ! videoscale ! " + rawCaps + " ! " + sinkProps);
        candidates.push_back(
            "pipewiresrc target-object=" + dev + " do-timestamp=true"
            " ! videoconvert ! videoscale ! " + rawCaps + " ! " + sinkProps);
        candidates.push_back(
            "pipewiresrc do-timestamp=true"
            " ! videoconvert ! videoscale ! " + rawCaps + " ! " + sinkProps);
    }
    ...
    for (const auto &pipelineStr : candidates) {
        ...
        if (parseErr != nullptr || pipeline == nullptr) {
            std::cerr << "[CameraCapture] parse error: "
                      << (parseErr != nullptr ? parseErr->message : "null pipeline") << "\n";
            if (parseErr != nullptr) { g_error_free(parseErr); }
            if (pipeline  != nullptr) { gst_object_unref(pipeline); }
            continue;
        }

        // Probe to READY — this allocates device resources without starting the
        // streaming thread. PLAYING validation is deferred to start() so that we
        // never need to tear down a live source mid-flight during candidate probing,
        // which can cause pipewiresrc to hang on cleanup.
        GstStateChangeReturn ret = gst_element_set_state(pipeline, GST_STATE_READY);
        GstState reached = GST_STATE_NULL;
        if (ret != GST_STATE_CHANGE_FAILURE) {
            ret = gst_element_get_state(pipeline, &reached, nullptr, GST_SECOND);
        }
        ...
        // Candidate failed — drain the bus for a diagnostic message then move on.
        GstBus *bus = gst_element_get_bus(pipeline);
        if (bus != nullptr) {
            GstMessage *msg = gst_bus_timed_pop_filtered(
                bus, 500 * GST_MSECOND, GST_MESSAGE_ERROR);
            if (msg != nullptr) {
                ...
                std::cerr << "\n";
                if (busErr != nullptr) { g_error_free(busErr); }
...
Result CameraCapture::start(std::function<void(std::unique_ptr<Frame>)> frameCallback) {
    ...
    if (ret == GST_STATE_CHANGE_FAILURE || state != GST_STATE_PLAYING) {
        logPipelineError(pipeline_);
        std::cerr << "[CameraCapture] pipeline failed to reach PLAYING state (reached: "
...
void CameraCapture::captureLoop(const std::stop_token &stopToken) {
    ...
    while (!stopToken.stop_requested()) {
        GstSample *sample = gst_app_sink_try_pull_sample(
            GST_APP_SINK(appsink_), kPullTimeoutNs);
        ...
        // Copy the three YUV planes row-by-row, guarding against stride mismatches.
        for (int plane = 0; plane < 3; ++plane) {
            ...
            const auto *src =
                static_cast<const uint8_t *>(GST_VIDEO_FRAME_PLANE_DATA(&vframe, plane));
        ...
        avf->pts = GST_CLOCK_TIME_IS_VALID(bufPts)
                       ? static_cast<int64_t>(bufPts)
                       : 0;
