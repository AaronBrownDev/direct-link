...
public:
    bool initialize(const std::string &device, bool useGPU = false) {
        ...
        if (avformat_open_input(&captureCtx, device.c_str(), inputFmt,
                                nullptr) < 0) {
        ...
        for (unsigned i = 0; i < captureCtx->nb_streams; i++) {
            if (captureCtx->streams[i]->codecpar->codec_type ==
                AVMEDIA_TYPE_VIDEO) {
                videoStreamIdx = i;
                break;
            }
        ...
        std::cout << "Found video stream: " << codecpar->width << "x"
                  << codecpar->height << "\n";
        ...
        if (std::string(encoder->name).find("nvenc") != std::string::npos) {
            ...
            std::cout
                << "Configured for CPU encoding (YUV420P, ultrafast preset)\n";
        }
    ...
    ~CaptureEncodeTransmit() {
        ...
        if (captureCtx)
            avformat_close_input(&captureCtx);
    }
...
private:
    void captureLoop() {
        ...
        while (running && !shouldExit) {
            if (av_read_frame(captureCtx, packet) >= 0) {
                if (packet->stream_index == videoStreamIdx) {
                    ...
                    // TODO: Decode if needed, or if raw just convert packet to
                    // frame
        ...
        std::cout << "Capture thread exiting (captured " << frameCount
                  << " frames)\n";
    }
    ...
    void encodeLoop() {
        ...
        av_packet_free(&packet);
        std::cout << "Encode thread exiting (encoded " << packetCount
                  << " packets)\n";
    }
    ...
    void transmitPacket(AVPacket *packet) {
        ...
        static int count = 0;
        if (++count % 60 == 0) {
            std::cout << "Transmitted " << count
                      << " packets (latest: " << packet->size << " bytes)\n";
        }
    }
};
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--gpu" || arg == "-g") {
            ...
            std::cout << "\nOptions:\n";
            std::cout
                << "  --cpu, -c     Use CPU encoding (libx264) [default]\n";
            ...
            std::cout << "  " << argv[0]
                      << "                    # CPU, /dev/video0\n";
            std::cout << "  " << argv[0]
                      << " --cpu /dev/video1  # CPU, /dev/video1\n";
            std::cout << "  " << argv[0]
                      << " --gpu /dev/video0  # GPU, /dev/video0\n";
            return 0;
        }
        else if (arg[0] != '-') {
            device = arg;
